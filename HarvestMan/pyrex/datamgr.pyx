...
from common import GetObject, MyDeque, info, moreinfo, extrainfo, \
     debug, plural, wasOrWere, bin_crypt, bin_decrypt

cdef class harvestManDataManager:
    """ The data manager cum indexer class """

    cdef int _numfailed
    cdef long _bytes
    cdef int _redownload

    cdef object _projectcache
    cdef object _downloaddict
    cdef object _cfg
    cdef object _linksdict
    cdef object _fetcherstatus
    cdef object _dataLock
    cdef object _urlThreadPool
    ...
    cdef get_proj_cache_filename(self):
    ...
    cdef get_links_dictionary(self):
    ...
    cdef update_cache_for_url(self, url, filename, contentlen, urldata, digeststr):
    ...
    cdef update_cache_for_url2(self, url, filename, lmt, urldata):
    ...
    cdef does_cache_need_update(self):
    ...
    cdef thread_download(self, urlObj):
    ...
    cdef check_duplicates(self, url):
    ...
    cdef add_headers_to_cache(self):
    ...
    cdef localise_file_links(self, filename, links):
    ...
    cdef print_project_info(self, statsd):
        ...
        infostr = infostr + 'files:'+str(nfiles)+','
        infostr = infostr + 'links:'+str(nlinks)+','
        infostr = infostr + 'dirs:'+str(ndirs)+','
        infostr = infostr + 'failed:'+str(numfailed)+','
        infostr = infostr + 'refetched:'+str(nretried)+','
        infostr = infostr + 'fatal:'+str(fatal)+','
        infostr = infostr + 'elapsed:'+str(fetchtime)+','
        infostr = infostr + 'fps:'+str(fps)+','
        infostr = infostr + 'bps:'+str(bps)+','
        infostr = infostr + 'timestamp:'+tstamp
        infostr = infostr + '\n'
    ...
    cdef dump_urltree(self, urlfile):
    ...
    cdef dump_urltree_textmode(self, stream):
    ...
    cdef dump_urltree_htmlmode(self, stream):
    ...
    def get_downloaddict(self):
        return self._downloaddict
...
class harvestManController(tg.Thread):
    ...
    def __manage_file_limits(self):
        ...
        ddict = self._dmgr.get_downloaddict()
