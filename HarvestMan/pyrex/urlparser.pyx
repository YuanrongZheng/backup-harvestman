...
cdef class HarvestManUrlParser:
    ...
    cdef object form_re
    cdef object junk_chars
    cdef object junk_chars_repl
    cdef object special_strings
    cdef object special_strings_repl
    ...
    def __new__(cls, *args, **kwargs):
        ...
        cls.junk_chars = ('?','*','"','<','>','!',':','/','\\')
        # Replacement chars
        cls.junk_chars_repl = ('',)*len(cls.junk_chars)

        # Special strings
        cls.special_strings = ('%20','%7E','%2B','%22','%3C','%3E','%23','%25',
                               '%7B','%7D','%7C','%5C','%5E','%5B','%5D','%60')

        # Special string replacements
        cls.special_strings_repl = (' ','~','+','"','<','>','#','%','{','}','|','\\','^','[',']','`')
