...
from common import GetObject,info,moreinfo,extrainfo,debug
# urlPathParser module
import urlparser

cdef class harvestManRulesChecker:
    ...
    cdef object _links
    cdef object _sourceurls
    cdef object _filter
    cdef object _extservers
    cdef object _extdirs
    cdef object _rexplist
    cdef char* _wordstr
    cdef object _robots
    cdef object _robocache
    cdef object _logger
    cdef object _configobj
    cdef object junkfilter
    ...
    def violates_basic_rules(self, urlObj):
       ...
       # now apply the url filter
        ...
        # depth check
        if self.__apply_depth_check(urlObj, 0):
    ...
    cdef __compare_domains(self, domain1, domain2, robots):
    ...
    cdef __get_base_server(self, server):
    ...
    cdef __compare_by_name(self, domain1, domain2):
    ...
    cdef __compare_by_ip(self, domain1, domain2):
    ...
    cdef __apply_url_filter(self, url):
    ...
    cdef __apply_server_filter(self, urlObj):
    ...
    cdef is_under_starting_directory(self, urlObj):
        ...
        # domain
        if self.__compare_domains(urlObj.get_domain(), baseUrlObj.get_domain(), False):
    ...
    cdef is_external_server_link(self, urlObj):
        ...
        return not self.__compare_domains( server, baseserver, False)

    cdef __is_external_link(self, urlObj):
        ...
        if not self.is_external_server_link(urlObj):
            ...
            if self._configobj.extdepth:
                if self.__apply_depth_check(urlObj, 2):
                    return True
    ...
    cdef __apply_depth_check(self, urlObj, mode):
    ...
    cdef __ext_directory_check(self, directory):
    ...
    cdef __ext_server_check(self, server):
    ...
    cdef __increment_ext_directory_count(self, directory):
    ...
    cdef __increment_ext_server_count(self,server):
    ...
    def make_filters(self):
        ...
        url_filters = self.__make_filter(urlfilterstr, 0)
        ...
        server_filters = self.__make_filter(serverfilterstr, 0)
    ...
    cdef __make_priority(self, pstr):
    ...
    cdef __make_filter(self, fstr,servers):
        ...
        exclusionfilter=self.__create_filter(rexclude,servers)
        inclusionfilter=self.__create_filter(rinclude,servers)
    ...
    cdef __create_filter(self, strlist, servers):
    ...
    cdef __parse_word_filter(self, s):
    ...
    cdef __is_inbetween(self, l, elem):
    ...
    cdef __make_word_filter(self, s):
    ...
    cdef __make_word_regexp(self, mylist):
...
cdef class JunkFilter:
    """ Junk filter class. Filter out junk urls such
    as ads, banners, flash files etc """

    cdef object block_domains
    cdef object block_patterns
    cdef char* msg
    cdef char* match
    cdef object patterns
    cdef object base_domains
    
    def __init__(self):
    
        # Domain specific blocking - List courtesy
        # junkbuster proxy.
        self.block_domains =[ '1ad.prolinks.de',
                              '1st-fuss.com',
                              '247media.com',
                              'admaximize.com',
                              'adbureau.net',
        ...
        self.block_patterns = [ r'/*.*/(.*[-_.])?ads?[0-9]?(/|[-_.].*|\.(gif|jpe?g))',
    ...
    cdef __check_domain(self, url_obj):
    ...
    cdef __check_pattern(self, url_obj):
