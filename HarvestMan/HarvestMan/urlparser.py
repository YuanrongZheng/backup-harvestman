...
class HarvestManUrlParser:
    ...
    image_extns = ('.bmp', '.dib', '.dcx', '.emf', '.fpx', '.gif', '.img',
    ...
    webpage_extns = ('.htm', '.html', '.shtml', '.php', '.php3',
                     '.asp', '.jsp','.psp','.pl','.cgi', '.stx')
    ...
    def get_url_directory(self):
        ...
        if self.dirpath:
            newpath = "".join((self.URLSEP, reduce(lambda x, y: x + '/' + y, self.dirpath)))
    ...
    def get_full_url_sans_port(self):
        ...
        if self.dirpath:
            newpath = reduce(lambda x, y: x + self.URLSEP + y, self.dirpath)
            rval = "".join((rval, self.URLSEP, newpath))

        if rval[-1] != '/': rval = "".join((rval, self.URLSEP))
    ...
    def get_relative_url(self):
        ...
        newpath=""
        if self.dirpath:
            newpath =  "".join(("/", reduce(lambda x, y: x + self.URLSEP + y, self.dirpath)))
    ...
    def get_relative_filename(self, filename=''):
        ...
        if prefix:
            ...
            # prefix. 
            prefix2 = os.path.commonprefix([dir,prefix])
            if prefix2[-1] != os.sep:
                prefix2 = "".join((prefix2, os.sep))
            
            # os.path.samepath is not avlbl in all
            # platforms.
            try:
                if os.path.samepath(dir, prefix2):
                    sub=True
            except:
                if dir==prefix2:
                    sub=True
            ...
            if sub:
                ...
                dirwithoutprefix = dir.replace(prefix, '')
    ...
    def get_relative_depth(self, hu, mode=0):
        ...
        if dir1[-1] != '/': dir1 = "".join((dir1, '/'))
        if dir2[-1] != '/': dir2 = "".join((dir2, '/'))
    ...
    def get_local_directory(self):
        ...
        for dir in self.dirpath:
            if dir:
                rval = os.path.abspath( os.path.join(rval, self.make_valid_filename(dir)))
...
if __name__=="__main__":
    ...
    for hu in hulist:
        ...
        print 'Local Url directory = ', hu.get_url_directory()
